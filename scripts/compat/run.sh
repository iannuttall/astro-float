...
for i in {1..60}; do
  if curl -s -o /dev/null -w '%{http_code}' http://127.0.0.1:$PORT/blog/hello-lee/ | grep -q 200; then break; fi
  sleep 1
done
...
if [ "${DOCTOR:-0}" = 1 ]; then echo "--- doctor ---"; node $ROOT/packages/astro-lee/src/cli.js doctor --url http://127.0.0.1:$PORT 2>&1 | head -40; fi
echo "--- server log (errors/warnings) ---"
cat $LOG .astro/dev.log 2>/dev/null | grep -inE "error|warn|cannot|failed|unsupported" | grep -v "x-lee" | head -20
