...
A proof-of-concept content editor for Astro content collections that lives **on the page**. In `astro dev` the rendered Markdown body is editable in place — click into the prose, type, save — and a thin dark rail (a vertical sibling of Astro's own dev toolbar pill) slides in at the right edge for the few things that can't live in the text: frontmatter fields, the collection, a read-only view of the Markdown that's about to hit disk, settings.
...
![Editing a blog post in place: caret in the text, a quiet dashed frame around the prose, the dark rail on the right with a white Save button](docs/float-editing.png)
...
Open <http://localhost:4321/blog/hello-float/>. The rail slides in on its own. Click into the article and type.
...
Hover the rail and it nudges out a little, like Astro's bottom bar; tooltips are Astro's too (dark, caret at the icon, slightly larger type). Panels open only when you click an icon and close when you navigate.
...
demo/                   a minimal Astro 5 blog + generic [collection] routes
docs/                   screenshots
```
...
- Prefs (dock side, autosave) live in `localStorage` under `astro-float:prefs`. Which panel is open is never persisted.
