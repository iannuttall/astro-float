...
- **Markdown** — the whole body as source, monospace. It opens on the block your caret was in on the page; the prose stays visible (read-only) and re-renders when you save (⌘S, or autosave). Switching away saves first and puts the caret back in that block; if the save fails the tab stays put with the error and a **Discard**. A small **copy** button at the end of the tab row copies the Markdown.
...
- **Markdown tab.** The body's source is the same draft the page edits. Opening the tab detaches the on-page editor (the prose stays visible, read-only); a save writes the text and swaps in Astro's fresh render under the panel. Saves are serialized, so switching tabs during an in-flight autosave waits for it rather than dead-clicking; a failed save keeps the tab with the error and a Discard; a saved-but-not-refreshed page falls back to the DOM it had, with a message. The page-side overlays (bubble, island bar) survive the swap — they're dev chrome, not page content.
- **Icons.** One set: Lucide paths at 1.75 stroke, round caps, 14–15px, in `toolbar/icons.ts`. Panel, bubble and island bar all draw from it.
...
packages/astro-float/   the integration (what you'd publish to npm)
  ...
  src/toolbar/overlays.ts the selection bubble
