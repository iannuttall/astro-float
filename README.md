...
- **Delete** — the popover's last line is a quiet **Delete post** (*note* for `notes`, *entry* when the collection's name doesn't make it obvious); click it and it becomes one question in place — *Delete “Hello, Float”? This removes its folder.* — with Cancel and Delete, where Esc or a click elsewhere puts it back and Enter only answers once Delete has the focus. With the entries open, the list ends with **Delete collection**, which asks the same way before removing the collection's folder, its `public/media/` folder and its `defineCollection()`; the page then moves on (the collection's listing, a neighbouring entry, or home) and the pill says **Deleted**.
