...
A flush column on the right edge — full height, no card, no gap. Light by default, dark when your system is; cool neutral greys, one hairline weight, one radius, 13px type with 12px help text; the only strong element is Save. Each view scrolls on its own without ever showing a scrollbar, so nothing changes width when a form gets taller. If a page has no editable body, the Fields view says so in one line at the top; the Markdown tab still edits it. **Drag its left edge** to resize (240px up to the smaller of half the viewport or 560px; the width is remembered in `localStorage`).
...
- **YAML** — the raw frontmatter, monospace with line numbers, long lines wrapped. It's parsed when you leave the text or switch tabs: valid YAML updates the same draft Fields shows; an error shows under the text and as a red dot on the tab, and the last valid frontmatter stays in force until you fix it.
- **Markdown** — the whole body as source, monospace. It opens on the block your caret was in on the page; the prose stays visible (read-only) and re-renders when you save (⌘S, or autosave). Switching away saves first and puts the caret back in that block; if the save fails the tab stays put with the error and a **Discard**. A small **copy** button at the end of the tab row copies the Markdown. The corner control on the body opens the same source in place instead; it's one state, so the two never fight.
