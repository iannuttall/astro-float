...
- **Markdown** — the whole body as source. It's the same source view the region control opens on the page, just docked: same draft, same save, and the page re-renders when you save (⌘S, or autosave). Switching away saves first; if that fails the tab stays put with the error and a **Discard**.
...
- **Icons.** One set: Lucide paths at 1.75 stroke, round caps, 14–15px, in `toolbar/icons.ts`. Panel, bubble, region control and island bar all draw from it.
