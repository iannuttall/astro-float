...
- **Changed on disk.** When an entry file changes and Float didn't write it (your editor, `git checkout`, a formatter), the server pushes `astro-float:file-changed` `{ collection, id, hash }` over Vite's HMR socket (`src/server/watch.js`; Float's own writes are known to the sync gate and skipped). The toolbar subscribes with `api.onFileChanged(cb)`; `hash` matches the one `/entry` returns, so a stale copy is easy to tell.
