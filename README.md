...
A proof-of-concept content editor for Astro content collections that lives **on the page**. Click **Edit** in the Astro Dev Toolbar and the rendered entry becomes editable where it sits — title, description, Markdown body. Anything editable takes a soft grey wash when you hover it; that's the whole visual language. A **panel** docks to the right for the few things that aren't on the page: the rest of the frontmatter as a form (or as YAML, or the body as Markdown), the collection, Save. Turn Edit off and both are gone.
...
![Edit on: the panel on the right with its Fields view, the title and body editable in place](docs/sidebar.png)
...
A flush column on the right edge — full height, no card, no gap. Light or dark to match the *site* (from the page's painted background, then its declared `color-scheme`, then your system preference); cool neutral greys, one hairline weight, one radius, 13px type with 12px help text; the only strong element is Save. Each view scrolls on its own without ever showing a scrollbar, so nothing changes width when a form gets taller. If a page has no editable body, the Fields view says so in one line at the top; the Markdown tab still edits it. **Drag its left edge** to resize (240px up to the smaller of half the viewport or 560px; the width is remembered in `localStorage`).

- **Header** — the entry, status, an **Autosave** switch, **Discard** / **Save**, and a **hide** button. Hiding tucks the panel away while editing carries on; a small tab at the edge brings it back and carries the status dot / Save in the meantime. Only the pencil turns Edit off.
- **Fields** (the default tab) — one row per frontmatter field: a humanized label (`pubDate` → "Pub date"), a line of help, and the right control for the field's type. Text, long text, number, toggle, date and date-time (the same calendar), a segmented control or a select for enums, tag chips, image (thumbnail, **Upload**, **Choose** from the images next to the entry), a nested group for objects, a list with add / remove / reorder for arrays, a select of that collection's entries for references, JSON for anything else. Types come from the collection's Zod schema when the dev server can read it, otherwise they're inferred from the values. Fields that are also on the page (title, description, date in the demo) say so and stay in sync both ways. `slug` is shown read-only, never edited. Nothing here is a trap: any field that differs from disk gets a **↺** to put it back; with an inferred schema you can add and remove fields, and a removed field stays listed with **Restore** until you save; an emptied number is never written — the last real value is kept (with a note) until you enter a valid one (a date can't be emptied: the calendar only ever yields a real day).
- **YAML** — the raw frontmatter, monospace with line numbers, long lines wrapped. Typing counts at once: the text is parsed a beat after you stop (and before any save), so ⌘S and autosave see it. Valid YAML updates the same draft Fields shows; an error shows under the text and as a red dot on the tab, the last valid frontmatter stays in force, and if you then edit a field elsewhere the broken text is dropped for a fresh render — a typo here never undoes an edit there.
- **Markdown** — the whole body as source, monospace. It opens on the block your caret was in on the page, and the page **previews as you type**: the prose (read-only while the tab is up) re-renders block by block a beat after each keystroke, without scrolling away from the block you're in. Nothing is written until you save (⌘S, Save, or autosave); switching tabs keeps your source edits and the page stays read-only until they're saved. A small **copy** button at the end of the tab row copies the Markdown. The corner control on the body opens the same source in place instead; it's one state, so the two never fight.
- **Footer** — the collection (dropdown if several), the entries behind a toggle with the current one marked, **New entry** (a title is all it asks; the derived path is shown) and **New collection** (creates `src/content/<name>/`, registers it in `content.config.ts`, seeds a first entry and opens it).

<p>
  <img src="docs/sidebar-hidden.png" width="480" alt="Panel hidden: a clean page with a small tab at the right edge, editing still on">
...
<p>
  <img src="docs/date-picker.png" width="800" alt="The calendar open under the article's date; the panel's date control shows the same value">
</p>
<p>
  <img src="docs/date-picker-sidebar.png" width="360" alt="The same calendar opened from the panel's date control">
</p>

On phones (<640px) the panel is a bottom sheet flush with the bottom edge, sized to the visual viewport so the keyboard shrinks it instead of hiding its buttons; inputs are 16px (no iOS focus-zoom); Cancel on a form keeps the list where it was and snaps back a lingering zoom.

<p>
  <img src="docs/mobile/sidebar.webp" width="200" alt="The panel as a bottom sheet on a phone">
...
- **Dev toolbar app.** `addDevToolbarApp()` registers "Edit" (only when `command === "dev"`). The panel renders into the app's canvas, so Astro hides it when the app is off; page-side styles (wash, body control, bubble, island bar) are injected for the edit session and removed after. Edit state persists across reloads in `sessionStorage`; `beforeTogglingOff` saves pending work first. Escape keyups are stopped at the document while editing so Astro's "Escape closes the app" never fires mid-edit.
...
- **Source view.** One source mode, two homes. From the corner control the body's rendered container is hidden and a textarea with the current Markdown takes its place; from the panel's Markdown tab the textarea is in the panel and the prose stays visible, read-only, previewing as you type: each pause posts the draft to `/__float/api/render` and the body's top-level children are swapped for the rendered blocks (islands keep the page's own element; a failed or slow render leaves the last good DOM). Either way a save writes the text and swaps in Astro's fresh render. Saves are serialized, so *Rendered* (or a tab switch) during an in-flight autosave waits for it rather than dead-clicking; a failed save keeps the source view with the error and a Discard; a saved-but-not-refreshed page falls back to the DOM it had, with a message. The page-side overlays (body control, bubble, island bar) survive the swap — they're dev chrome, not page content.
