...
- **Schema.** Float doesn't parse `content.config.ts`. `astro sync` (run by `astro dev` at start-up and on every config change) writes a JSON Schema per collection to `.astro/collections/<name>.schema.json`; `src/server/schema.js` reads that on every entry load and flattens it into field kinds (`z.coerce.date()` → date, `z.enum()` → select, `z.array(z.string())` → tags, `.optional()` / `.default()` / `.describe()` / `.nullable()` carried through), so the sidebar understands the schema exactly the way Astro does. No file → value inference.
