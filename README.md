...
- **Fields** — the whole frontmatter as a form. With a **schema** (a `schema:` in `content.config.ts`) the form follows it: fields in schema order with the control the type calls for — text, long text, number (`.int()` / `.min()` / `.max()` respected, with a note when the value is outside them), date (calendar), tags, boolean, a **select** for `z.enum()`, a select of that collection's entries for `reference("blog")`, JSON for objects — then anything else the file has, marked *not in schema*. Required fields say so (and can't be removed); a schema field the file doesn't have is listed with **Add**, which inserts its default or an empty value of the right shape; `.describe()` text shows under the key; a required field left empty gets a note (the file still saves — Astro is what would complain). Without a schema, controls are inferred from the values, as before. Fields that are also on the page (title, description, date in the demo) say so and stay in sync both ways. `slug` is shown read-only, never edited. Nothing here is a trap: any field that differs from disk gets a **↺** to put it back; a removed field stays listed with **Restore** until you save; an emptied number is never written — the last real value is kept (with a note) until you enter a valid one (a date can't be emptied: the calendar only ever yields a real day); a value that doesn't fit its schema type is edited as what it is, with a note, never silently coerced.
- **Collection** — entries in the selected collection (dropdown if several), current one marked; **New entry** asks for a title only and shows the derived path (its frontmatter starts from the schema: defaults and required fields in schema order, topped up from what sibling entries use); **New collection** creates `src/content/<name>/`, registers it in `content.config.ts`, seeds a first entry and opens it.
...
- **Schema.** Float doesn't parse `content.config.ts`. `astro sync` (run by `astro dev` at start-up and on every config change) writes a JSON Schema per collection to `.astro/collections/<name>.schema.json`; `src/server/schema.js` reads that on every entry load and flattens it into field kinds (`z.coerce.date()` → date, `z.enum()` → select, `z.array(z.string())` → tags, `reference()` → entry picker, `.int()` / `.min()` / `.max()`, `.optional()` / `.default()` / `.describe()` / `.nullable()` carried through), so the sidebar understands the schema exactly the way Astro does. The one thing read out of `content.config.ts` itself is which collection a `reference()` points at — the JSON Schema doesn't keep that. No file → value inference.
- **API.** `astro:server:setup` mounts `/__float/api/*`: collections, read/write entry (with the collection's schema), create entry, create collection, upload media. Localhost-only (host + socket address), same-origin, custom header on mutations, paths confined to the collection dir, image extension allowlist. Saves carry the file hash as loaded; a 409 gives you Reload / Overwrite.
...
packages/astro-float/   the integration (what you'd publish to npm)
  ...
  src/server/schema.js    the collection's Zod schema, read from Astro's .astro/collections/*.schema.json
...
demo/                   a minimal Astro 5 blog (+ one .mdx post), a `notes` collection whose schema
                        uses z.enum / .optional / .describe / reference("blog"), + generic [collection] routes
