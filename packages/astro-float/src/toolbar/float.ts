...
class Float {
   ...
   * (the region control's Source view, standing in for the prose) or in the
   * panel (the Markdown tab). Same draft, same save either way.
   */
  ...
  constructor(
    ...
    this.panel = new Panel(this.root, {
      ...
      openSource: (area) => this.enterSource(area, false),
  ...
  private onFocusIn = (e: FocusEvent) => {
    ...
    if (!target) return;
    if (this.sourceArea && this.sourceInPage && target === this.sourceArea) {
  ...
  private bodyRegionActions() {
    return {
      ...
      isSource: () => !!this.sourceArea && this.sourceInPage,
  ...
  /** Region control's Source / Rendered button: swap the prose for a Markdown textarea in the same spot (and back). */
  private async toggleSourceMode() {
    ...
    if (this.sourceArea) {
      ...
      const place = this.sourceInPage ? this.sourcePlace(this.sourceArea) : null;
      const left = await this.leaveSource();
      if (left && place && this.doc && this.page.bound && !this.bodyReadOnly) {
   ...
   * the panel the prose stays visible (read-only) and re-renders on save.
   * Moving between the two keeps the draft.
   */
  private enterSource(area: HTMLTextAreaElement, inPage: boolean) {
  ...
  /** Leave source mode: a save re-renders the page from what was typed. False when the save didn't take. */
