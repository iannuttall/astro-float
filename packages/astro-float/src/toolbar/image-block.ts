...
/** The declarations Float writes on a wrapper; any other one belongs to the site. */
...
/**
 * The wrapper's `style` for a layout (null: none of Float's), keeping any
 * declaration that isn't Float's (a site may have added its own). Float's own
 ...
 * wrapper the site wrote itself is never taken away, only Float's declarations
...
export function applyImageLayout(block: ImageBlock, layout: ImageLayout): HTMLElement {
  ...
  if (layout.size >= 100) {
    ...
    if (!rest && isFloatWrapper(wrapper)) {
...
/** Nothing on the `<div>` but a style and the editor's own marks: the wrapper Float writes. */
function isFloatWrapper(el: HTMLElement): boolean {
  return Array.from(el.attributes).every((a) => a.name === "style" || a.name === "contenteditable" || a.name === "draggable" || a.name.startsWith("data-float-"));
}
