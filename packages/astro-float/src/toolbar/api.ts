...
/** One problem the collection's Zod schema found with a value; `path` is the key path (`["priority"]`, `["hero", "src"]`). */
...
export class ApiError extends Error {
  status: number;
  /** Present on a 422: the schema refused the save; nothing was written. */
...
async function request<T>(path: string, init: RequestInit = {}): Promise<T> {
  ...
  if (!res.ok) {
    ...
    const issues = Array.isArray(body?.issues) ? (body.issues as ValidationIssue[]) : undefined;
