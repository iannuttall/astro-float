#!/usr/bin/env node
/**
 * astro-lee — the command line.
 *
 *   astro-lee doctor [--url http://localhost:4321] [--route /blog/[id]] [--all]
 *
 * With `astro dev` running, `doctor` asks Lee's API about every collection
 ...
 * what Lee needs to find them without `data-lee-*` attributes. Exit code
...
if (command === "doctor") {
  doctor(parseFlags(args)).then(
    ...
    (err) => {
      console.error(`astro-lee: ${err?.message ?? err}`);
      process.exit(2);
    },
  );
} else {
  console.log(
    [
      "astro-lee — Lee, the Little Entry Editor for Astro (dev only)",
      "",
      "Usage:",
      "  astro-lee doctor [--url http://localhost:4321] [--route /blog/[id]] [--all]",
      "",
...
async function checkPage({ api, base, collection, entry, route }) {
  ...
  for (const [key, value] of Object.entries(doc.frontmatter ?? {})) {
    ...
    if (type === "tags") {
      ...
      if (!ok) fixes.push(`add data-lee-field="${key}" to the element that lists the tags, or print every tag verbatim`);
      continue;
    }
    if (type === "date" || type === "datetime") {
      ...
      if (!formatted) fixes.push(`add data-lee-field="${key}" to the <time> (or element) that prints the date`);
      continue;
    }
    const ok = has(String(value));
    marks.push({ label: key, ok });
    if (!ok) fixes.push(`add data-lee-field="${key}" to the element that prints it, or print the value verbatim`);
  }
  ...
  if (first) {
    ...
    if (!ok) fixes.push("add data-lee-body to the element that wraps the rendered Markdown");
  } else {
