...
async function imageStubFactory(ctx) {
  ...
  return () => {
    ...
    const marker = { _leeImage: true, _def: { typeName: "ZodString" } };
 ...
 * ("optional", …) with different child keys. Lee's own `imageStub()` nodes
...
function unwrapZod(schema, depth = 0) {
  ...
  if (schema._leeImage) return schema;
...
function probeReference(effects) {
  try {
    const result = effects.safeParse("__lee_probe__");
