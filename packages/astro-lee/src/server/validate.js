...
/**
 * Parse the frontmatter with Astro's own parser (js-yaml: dates become Dates).
 * Astro 5 ships it in `@astrojs/markdown-remark`; Astro 6+ moved it to
 * `@astrojs/internal-helpers/frontmatter` (and 7 no longer installs
 * markdown-remark at all). Our YAML reader when neither resolves.
 */
async function frontmatterAsAstroReadsIt(root, document) {
  let parseFrontmatter = null;
  for (const specifier of ["@astrojs/markdown-remark", "@astrojs/internal-helpers/frontmatter"]) {
    try {
      ({ parseFrontmatter } = await importFromAstro(root, specifier));
      if (typeof parseFrontmatter === "function") break;
    } catch {
      /* not resolvable from this project's astro */
    }
  }
  if (typeof parseFrontmatter === "function") return parseFrontmatter(document, { frontmatter: "remove" }).frontmatter ?? {};
