...
import { attachLeeApi } from "./server/api.js";
...
/**
 * Lee (astro-lee), the Little Entry Editor: on-page editing for Astro content collections.
 *
 ...
 * holds the rest. Localhost-only JSON endpoints under `/__lee/api/*` read and
 * write the Markdown files on disk. Nothing is added to production builds.
 *
 * @param {import('../index.js').LeeOptions} [options]
 * @returns {import('astro').AstroIntegration}
 */
export default function lee(options = {}) {
  ...
  return {
    ...
    hooks: {
      "astro:config:setup": ({ command, config, addDevToolbarApp, updateConfig, logger }) => {
        ...
        updateConfig({ vite: { optimizeDeps: { include: ["astro-lee > yaml"] } } });
      ...
      "astro:server:setup": ({ server, logger }) => {
        ...
        attachLeeApi(server, ctx);
        // Outside edits to entry files reach the toolbar as `astro-lee:file-changed`.
