...
export function blockToMarkdown(node: Node, ctx: SerializeContext): string {
  ...
  if (el.hasAttribute("data-lee-island")) return rawHtml(el, ctx);
...
/**
 * A sized / aligned image: the wrapper's own tag and attributes (minus Lee's),
...
function wrapperToMarkdown(wrapper: HTMLElement, paragraph: HTMLElement, ctx: SerializeContext): string {
  ...
  const attrs = Array.from(wrapper.attributes)
    .filter((a) => !isLeeAttribute(a.name))
    .map((a) => (a.value === "" ? ` ${a.name}` : ` ${a.name}="${a.value.replace(/&/g, "&amp;").replace(/"/g, "&quot;")}"`))
    .join("");
  return `<${tag}${attrs}>\n\n${finishInline(inlineOf(paragraph, ctx))}\n\n</${tag}>`;
}

function isLeeAttribute(name: string): boolean {
  return name === "contenteditable" || name === "draggable" || name.startsWith("data-lee-");
}

/** The element as it was written, minus Lee's editing attributes; media `src`s go back to what belongs in the file. */
