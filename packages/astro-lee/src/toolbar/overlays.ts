...
export class RegionControl {
  ...
  private onEnter = () => {
    this.target?.setAttribute("data-lee-hover", "");
    this.paint();
  };

  private onLeave = () => {
    this.target?.removeAttribute("data-lee-hover");
    this.paint();
  ...
  private unbindTarget() {
    const t = this.target;
    if (!t) return;
    t.removeAttribute("data-lee-hover");
  ...
  private paint = () => {
    ...
    const over = t.matches(":hover") || t.hasAttribute("data-lee-hover");
  ...
  private render() {
    ...
    if (error && isSource) {
      ...
      note.className = "lee-region-error";
...
/** The page's painted background behind an element; white or near-black by Lee's theme when nothing paints one. */
function pageBackground(el: Element): Rgba {
  for (let node: Element | null = el; node; node = node.parentElement) {
    const c = parseColor(getComputedStyle(node).backgroundColor);
    if (c && c.a >= 0.99) return c;
  }
  return document.documentElement.getAttribute("data-lee-theme") === "dark" ? { r: 15, g: 17, b: 20, a: 1 } : { r: 255, g: 255, b: 255, a: 1 };
}
...
export class SelectionBubble {
  ...
  private currentRange(): Range | null {
    ...
    if ((range.commonAncestorContainer as Element).closest?.("[data-lee-island]")) return null;
