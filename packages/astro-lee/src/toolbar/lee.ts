...
export function mountLee(canvas: ShadowRoot, host: LeeHost): LeeHandle {
  const lee = new Lee(canvas, host);
  return {
    setEditing: (on) => lee.setEditing(on),
    beforeEditOff: () => lee.beforeEditOff(),
  };
}
...
class Lee {
  ...
  /** Body draft used only when the page has no `[data-lee-body]` to edit in place. */
  private draftBody = "";
  ...
  constructor(
    ...
    private host: LeeHost,
  ) {
    const style = document.createElement("style");
    style.textContent = STYLES;
    canvas.appendChild(style);

    this.root = h("div", { class: "lee" });
    ...
    // Only real unloads (tab close, hard refresh) should ever warn — Lee's own
    ...
    window.addEventListener("popstate", (e) => {
      if (this.editing || (e.state && (e.state as { lee?: boolean }).lee)) void this.navigate(location.href, { push: false });
    });

    // Read-only introspection for tests and bug reports: host.__lee.state()
    (this.canvas.host as HTMLElement & { __lee?: unknown }).__lee = {
      state: () => ({
  ...
  private applyTheme() {
    ...
    if (document.documentElement.getAttribute("data-lee-theme") !== theme) {
      document.documentElement.setAttribute("data-lee-theme", theme);
  ...
  private unwatchTheme() {
    ...
    document.documentElement.removeAttribute("data-lee-theme");
  }
  ...
  private async navigate(href: string, { push = true, focusBody = false, discard = false } = {}) {
    ...
    try {
      ...
      if (push) history.pushState({ lee: true }, "", url.href);
      window.scrollTo(0, 0);
      await this.loadPage();
      if (focusBody) this.page.focusStart();
      this.syncRegion();
    } catch (err) {
      console.warn("[astro-lee] soft navigation failed, falling back to a full load", err);
  ...
  /**
   * The dev server saw this entry's file change outside Lee. A clean draft
    ...
    try {
      ...
      if (result.changed && refresh) {
        ...
        try {
          ...
          console.warn("[astro-lee] page refresh failed", err);
