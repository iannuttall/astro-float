 ...
 * change goes back through it. Lee keeps the draft, the doc and the saving.
 */
...
export class Pill {
  ...
  private onDocumentPointerDown = (e: PointerEvent) => {
    ...
    if (path.some((n) => n instanceof HTMLElement && n.classList.contains("lee-datepicker"))) return;
    this.close();
  };
  ...
  renderFields() {
    ...
    if (!doc) {
      replaceChildren(
        this.fieldsHost,
        h("p", { class: "empty" }, "Open an entry to edit it. If Lee can't detect one from the URL, bind the page with ", h("code", null, 'data-lee-entry="blog:my-post"'), "."),
      );
  ...
  renderStatus(view: StatusView) {
    ...
    const tip = view.tip ?? (view.dot === "dirty" ? "Unsaved changes · ⌘S to save" : view.text || (this.host.doc() ? "Up to date" : "Lee"));
    if (this.pillEl) {
