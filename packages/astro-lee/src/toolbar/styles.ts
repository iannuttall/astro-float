...
/* Dark: chosen from the *site's* colour scheme (background luminance / color-scheme) by Lee,
...
.lee *, .lee *::before, .lee *::after { box-sizing: border-box; }
.lee :where(button, input, select, textarea) { font: inherit; color: inherit; margin: 0; letter-spacing: inherit; }
.lee :where(button) { cursor: pointer; background: none; border: 0; padding: 0; text-align: left; }
.lee :focus-visible { outline: 2px solid var(--line-focus); outline-offset: 1px; }
.lee :where(a) { color: inherit; text-decoration: none; }
.lee svg { flex: none; }
.lee [hidden] { display: none !important; }
...
.pill-dot[data-state="saving"] { background: var(--fg-faint); animation: lee-pulse 900ms ease-in-out infinite; }
.pill-dot[data-state="saved"] { background: var(--ok); }
.pill-dot[data-state="error"], .pill-dot[data-state="conflict"] { background: var(--err); }
@keyframes lee-pulse { 50% { opacity: 0.3; } }
...
.lee .address-input:focus-visible { outline: none; }
...
.btn-primary { background: var(--accent); color: var(--accent-fg); border-color: var(--accent); animation: lee-in 160ms ease; }
...
@keyframes lee-in { from { opacity: 0; } }
...
.date-btn[data-lee-open] { border-color: var(--line-focus); box-shadow: 0 0 0 3px color-mix(in srgb, var(--line-focus) 22%, transparent); }
