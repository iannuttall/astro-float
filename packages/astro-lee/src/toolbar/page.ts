...
const ROUTES_KEY = "astro-lee:routes";
 ...
 *
 * 1. An explicit binding wins: `<article data-lee-entry="blog:hello-world">`.
...
export function detectEntry(collections: Collection[]): DetectedEntry | null {
  const bound = document.querySelector<HTMLElement>("[data-lee-entry]")?.dataset.leeEntry;
  if (bound) {
...
const KEEP_SELECTOR =
  "astro-dev-toolbar, [data-lee-host], .lee-bubble, .lee-bar, .lee-frame, .lee-dropline, .lee-grip, .lee-datepicker";

/**
 * Render a page in place: fetch its HTML from the dev server and swap
 * everything except dev-tooling elements (the Astro toolbar and Lee itself).
