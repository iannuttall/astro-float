...
describe("PageEditor round trip", () => {
  ...
  it("moves an island's source verbatim when its DOM node moves", async () => {
    const { editor, container } = await bound(BODY);
    const island = container.querySelector("[data-lee-island]") as HTMLElement;
  ...
  it("inserts a raw-HTML island whose source is written verbatim", async () => {
    ...
    expect(island.hasAttribute("data-lee-island")).toBe(true);
  ...
  it("treats a rendered image paragraph and a sized-image wrapper as image blocks, and writes the wrapper back verbatim", async () => {
    ...
    expect(blocks.map((b) => [b.tagName, b.hasAttribute("data-lee-image"), b.hasAttribute("data-lee-key")])).toEqual([
      ["P", false, false],
      ["P", true, false],
      ["DIV", true, false],
      ["P", false, false],
  ...
  it("takes every image mark off the DOM on unbind", async () => {
    ...
    expect(container.querySelectorAll("[data-lee-image]").length).toBe(2);
    editor.attach();
    editor.unbind();
    expect(container.querySelector("[data-lee-image], [data-lee-island], [contenteditable], [draggable]")).toBeNull();
    expect(container.innerHTML).toBe(before);
  });
  ...
  it("sizes a picture and puts it back: the wrapper is written, and 100% brings back the original bytes and a clean state", async () => {
    const body = "Before.\n\n![pic](./x.png)\n\nAfter.\n";
    const { editor, container } = await bound(body);
    const picture = container.querySelector<HTMLElement>("[data-lee-image]")!;
    editor.setImageLayout(picture, { size: 50, align: "center" });
    const wrapper = container.querySelector<HTMLElement>(":scope > div[data-lee-image]")!;
    expect(picture.hasAttribute("data-lee-image")).toBe(false);
  ...
  it("makes a paragraph that became a lone picture an image block once the caret has left it", async () => {
    ...
    expect(p.hasAttribute("data-lee-image")).toBe(false);
    caret(next.firstChild!, 0);
    next.append("!");
    await new Promise((r) => setTimeout(r));
    expect(p.hasAttribute("data-lee-image")).toBe(true);
