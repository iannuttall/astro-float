...
async function roundTrip(md: string): Promise<string> {
  ...
  if (blocks.length === container.childNodes.length) {
    blocks.forEach((b, i) => {
      if (b.island) (container.childNodes[i] as HTMLElement).setAttribute("data-lee-island", "");
    });
  }
...
describe("islands and raw HTML", () => {
  it("writes islands verbatim, minus Lee's editing attributes, with media paths mapped", () => {
  ...
  it("never turns an island's blockquote into `>` syntax", () => {
    const el = containerFrom('<blockquote data-lee-island="">quote</blockquote>').firstElementChild!;
...
describe("image blocks", () => {
  ...
  const decorated = 'contenteditable="false" draggable="true" data-lee-island="" data-lee-image=""';
  ...
  it("writes a sized / aligned image as its wrapper around the Markdown image, minus Lee's attributes", () => {
  ...
  it("does not mistake other divs for image blocks", () => {
    expect(one('<div data-lee-island=""><p><img src="a.png" alt="a"></p><p>text</p></div>')).toBe('<div><p><img src="a.png" alt="a"></p><p>text</p></div>');
    expect(one('<div data-lee-island=""><p>caption <img src="a.png" alt="a"></p></div>')).toBe('<div><p>caption <img src="a.png" alt="a"></p></div>');
